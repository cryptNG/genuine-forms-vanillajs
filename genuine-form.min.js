const{isArray:a}=Array;export default class GenuineForm extends HTMLElement{secret=null;solution=null;isVerifiedCaptcha=!1;timerId=null;gfApiUrl='https://genuine-forms.io/api/gf-send-dev/';handleSendResponse=new Map;handleStartSending=new Map;handleFinishedSending=new Map;handleValidationFailed=new Map;handleValidateForm=(A,b)=>d(A,b);handleInitialized=(_,B)=>console.log('Default handleInitialized:',_,B);generateSubjectAndBody=(C,_b,_c='Generic Subject')=>({subject:_c,body:JSON.stringify(e(_b))});constructor(){super();this.name=this.getAttribute('name')||'genuine-form';this.subject=this.getAttribute('subject')||'Generic Subject';this._apiKey=this.getAttribute('api-key');this.receiver=this.getAttribute('receiver');this.gfApiUrl=this.getAttribute('api-url')||this.gfApiUrl;this.abortController=new AbortController;this.genuineCaptchaNode=null;this.events={on:(_A,_B)=>{let E=async (...aA)=>{try{await _B(...aA)}catch(aB){console.error(`Error in ${_A} handler:`,aB)}};_A==='send-response'&&this.handleSendResponse.set(_B,E);_A==='started-sending'&&this.handleStartSending.set(_B,E);_A==='finished-sending'&&this.handleFinishedSending.set(_B,E);_A==='validation-failed'&&this.handleValidationFailed.set(_B,E)},off:(aC,aD)=>{aC==='send-response'&&this.handleSendResponse.delete(aD);aC==='started-sending'&&this.handleStartSending.delete(aD);aC==='finished-sending'&&this.handleFinishedSending.delete(aD);aC==='validation-failed'&&this.handleValidationFailed.delete(aD)}};!window.genuineForms&&(window.genuineForms={});let _a=document.getElementById('genuine-form'),_C=document.createElement('style');if(!_a){console.error('Template #genuine-form not found');return}let D=_a.content;this.attachShadow({mode:'open'});_C.textContent=`
          :host{
            --form-display:flex;
