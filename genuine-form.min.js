const{isArray:a}=Array;export default class GenuineForm extends HTMLElement{secret=null;solution=null;isVerifiedCaptcha=!1;timerId=null;gfApiUrl='https://genuine-forms.io/api/gf-send-dev/';handleSendResponse=new Map;handleStartSending=new Map;handleFinishedSending=new Map;handleValidationFailed=new Map;handleValidateForm=(_,b)=>d(_,b);handleInitialized=(B,C)=>console.log('Default handleInitialized:',B,C);generateSubjectAndBody=async (_a,_b,_c='Generic Subject')=>({subject:_c,body:JSON.stringify(A(_b)),payload:JSON.stringify(await f(_b))});constructor(){super();this.payload={};this.name=this.getAttribute('name')||'genuine-form';this.subject=this.getAttribute('subject')||'Generic Subject';this._apiKey=this.getAttribute('api-key');this.receiver=this.getAttribute('receiver');this.gfApiUrl=this.getAttribute('api-url')||this.gfApiUrl;this.abortController=new AbortController;this.genuineCaptchaNode=null;this.events={on:(_A,E)=>{let aA=async (...aB)=>{try{await E(...aB)}catch(aC){console.error(`Error in ${_A} handler:`,aC)}};_A==='send-response'&&this.handleSendResponse.set(E,aA);_A==='started-sending'&&this.handleStartSending.set(E,aA);_A==='finished-sending'&&this.handleFinishedSending.set(E,aA);_A==='validation-failed'&&this.handleValidationFailed.set(E,aA)},off:(aD,aE)=>{aD==='send-response'&&this.handleSendResponse.delete(aE);aD==='started-sending'&&this.handleStartSending.delete(aE);aD==='finished-sending'&&this.handleFinishedSending.delete(aE);aD==='validation-failed'&&this.handleValidationFailed.delete(aE)},call:(aF,...aG)=>{aF==='set-payload'&&this.setPlayload(...aG);aF==='unset-payload'&&this.unsetPlayload(...aG)}};!window.genuineForms&&(window.genuineForms={});let D=document.getElementById('genuine-form'),_C=document.createElement('style');if(!D){console.error('Template #genuine-form not found');return}let _B=D.content;this.attachShadow({mode:'open'});_C.textContent=`
          :host{
            --form-display:flex;
